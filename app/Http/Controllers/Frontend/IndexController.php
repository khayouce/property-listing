...
class IndexController extends Controller
{
    public function PropertyDetails($id,$slug){
        ...
        $property_amenit = explode(',',$amenities); 
        ...
        return view('frontend.property.property_details',compact('property','multiImage','property_amenit','facility','relatedProperty'));
    }
    ...
    public function PropertyMessage(Request $request){
        ...
        if (Auth::check()) {
            ...
            $notification = array(
                'message' => 'Login Your Account to Send Message',
                'alert-type' => 'error'
            );
            return redirect()->back()->with($notification);
        }
    ...
    public function StateDetails($id){
        ...
        $sbread = State::where('id',$id)->first();
        $rentproperty = Property::where('property_status','rent')->get();
        $buyproperty = Property::where('property_status','buy')->get();
        return view('frontend.property.state_property',compact('property','sbread','rentproperty','buyproperty'));
    }
    ...
    public function BuyPropertySearch(Request $request){
        ...
        $property = Property::where('status','1')->where('property_name', 'like' , '%' .$item. '%')->where('property_status','buy')->with('type','pstate')->whereHas('pstate', function($q) use ($sstate){
    ...
    public function RentPropertySearch(Request $request){
        ...
        $property = Property::where('status','1')->where('property_name', 'like' , '%' .$item. '%')->where('property_status','rent')->with('type','pstate')->whereHas('pstate', function($q) use ($sstate){
            $q->where('state_name', 'like' , '%' .$sstate. '%');
        })
        ->whereHas('type', function($q) use ($stype){
            $q->where('type_name', 'like' , '%' .$stype. '%');
    ...
    public function StoreSchedule(Request $request){
        ...
        if (Auth::check()) {
            ...
            $notification = array(
                'message' => 'You Successfully Schedule A Tour',
                'alert-type' => 'success'
            );
            return redirect()->back()->with($notification);
            ...
            $notification = array(
                'message' => 'Login Your Account To Schedule A Tour',
                'alert-type' => 'error'
            );
            return redirect()->back()->with($notification);
        }
